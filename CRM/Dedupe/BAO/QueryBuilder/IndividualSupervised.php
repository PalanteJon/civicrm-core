...
class CRM_Dedupe_BAO_QueryBuilder_IndividualSupervised extends CRM_Dedupe_BAO_QueryBuilder {
  ...
  public static function record($rg) {
    ...
    return array(
      "civicrm_contact.{$rg->name}.{$rg->threshold}" => CRM_Core_DAO::composeQuery("
                ...
                WHERE contact_type = 'iIndividual'
                  AND ((first_name = %1
                    AND last_name = %2)
                    OR email = %3)", $params, TRUE),
    );
  }
  ...
  public static function internal($rg) {
    $query = self::filterQueryByContactList($rg->contactIds, "
            ...
            WHERE contact1.contact_type = 'Individual'") . "UNION" .
             self::filterQueryByContactList($rg->contactIds, "
            SELECT contact1.id as id1, email2.contact_id as id2, {$rg->threshold} as weight
            FROM civicrm_contact as contact1
              JOIN civicrm_email email1 ON contact1.id = email1.contact_id
              JOIN civicrm_email email2 ON email1.email = email2.email
            WHERE contact1.contact_type = 'Individual'", 'contact1.id', 'email2.contact_id');
