...
WHERE    civicrm_participant.contact_id = {$contactID} AND
  ...
  public static function getSelfServiceEligibility(int $participantId, string $url, bool $isBackOffice) : array {
    ...
    if (!$details['allow_selfcancelxfer']) {
      ...
      $details['ineligible_details'] = ts('This event registration can not be transferred or cancelled. Contact the event organizer if you have questions.');
      return $details;
    }
    //verify participant status is still Registered
    if ($details['status'] != 'Registered') {
      $details['eligible'] = FALSE;
      $details['ineligible_details'] = "You cannot transfer or cancel your registration for " . $eventTitle . ' as you are not currently registered for this event.';
      return $details;
    }
    ...
    if (!$isBackOffice && !empty($time_limit)) {
      $deadlineIsNegative = $time_limit < 0 ? 1 : 0;
      $cancelHours = abs($time_limit);
      $cancelInterval = new DateInterval("PT${cancelHours}H");
      $cancelInterval->invert = $deadlineIsNegative;
      ...
      if ($timenow > $cancelDeadline) {
        ...
        $afterOrPrior = $deadlineIsNegative ? 'after' : 'prior';
        $moreOrLess = $deadlineIsNegative > 0 ? 'more' : 'less';
        $details['ineligible_details'] = ts("Registration for this event cannot be cancelled or transferred %2 than %1 hours %3 to the event's start time. Contact the event organizer if you have questions.",
        [1 => $cancelHours, 2 => $moreOrLess, 3 => $afterOrPrior]);
      }
    }
    return $details;
  }
